        ...
        if (errorObject.response && errorObject.response.data) {
            ...
            if (errors && errors.length > 0 ) {
                ...
                Object.values(errors).map((item, key)=>{
                    ...
                    return shouldDispatchAlert && store.dispatch(setAlertAction({type:'warning', message:item.toString()}));
                });
                ...
                shouldDispatchAlert && store.dispatch(setAlertAction({type:'warning', message: message}));
            }
            ...
            if (errorObject.message === 'Network Error'){
                ...
                // Send found errors to alert
                shouldDispatchAlert && store.dispatch(setAlertAction({type:'danger', message:'Network is Unavailable'}));

            } else {

                // Send generic error message to alert
                shouldDispatchAlert && store.dispatch(setAlertAction({type:'danger', message:'Server is Unavailable'}));
            }
            ...
            shouldDispatchAlert && store.dispatch(setAlertAction({type:'info', message:'Application is Unavailable'}));
